...
elseif h_ref >= h_max
    ...
    depths = [h_ref:-scan_step:h_min, (h_ref+scan_step):scan_step:h_max];
end
...
for i = 1:length(depths)
    ...
    [stab, ~, ~, TPD_min, conv] = stability_analysis_ssi(comp_h, P_h, T_h, Pc, Tc, acentric, BIP, ssi_tol, ssi_maxiter, eos_type, components);
    ...
    if ~isnan(stab_prev) && stab_prev == 1 && stab == 2
        ...
        [GOC_depth, GOC_pressure, GOC_comp, GOC_temp] = bisect_goc( ...
            ...
            vt_method, vt_params, tau, eos_type, ssi_tol, ssi_maxiter, tol_depth, components);
...
function [comp_h, P_h, T_h] = call_grading(model_type, h, h_ref, comp_ref, P_ref, T_ref, dTdh, Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vt_method, vt_params, tau)
    ...
    switch model_type
        case 0
            [comp_h, P_h, T_h, ~, ~] = main_hasse(h, h_ref, comp_ref, P_ref, T_ref, 0, ...
...
function [h_goc, P_goc, comp_goc, T_goc] = bisect_goc(model_type, h_stable, h_unstable, h_ref, comp_ref, P_ref, T_ref, dTdh, Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vt_method, vt_params, tau, eos_type, ssi_tol, ssi_maxiter, tol_depth, components)
...
for iter = 1:50
    ...
    [stab_mid, ~, ~, TPD_mid, ~] = stability_analysis_ssi(comp_mid, P_mid, T_mid, Pc, Tc, acentric, BIP, ssi_tol, ssi_maxiter, eos_type, components);
