...
for i = 1:nc
    if i <= length(components) && ~isempty(components{i})
        [params_db, found] = abudour_parameters(strtrim(components{i}));
        if found
            c1(i) = params_db.c1;
            Zc(i) = params_db.Zc;
        else
...
Z_eos = press * V_EOS / (R * temp);
A_param = a_mix * press / (R * temp)^2;
B_param = b_mix * press / (R * temp);

fugcoef = zeros(nc, 1);
for i = 1:nc
    sum_xaij = 0;
    for j = 1:nc
        sum_xaij = sum_xaij + comp(j) * sqrt(a_coef(i)*a_coef(j)) * (1 - BIP(i,j));
    end
    if strcmp(eos_type, 'SRK')
        ln_phi = (b_coef(i)/b_mix)*(Z_eos-1) - log(Z_eos-B_param) ...
            - (A_param/B_param)*(2*sum_xaij/a_mix - b_coef(i)/b_mix)*log(1 + B_param/Z_eos);
    else
        sqrt2 = sqrt(2);
        ln_phi = (b_coef(i)/b_mix)*(Z_eos-1) - log(Z_eos-B_param) ...
            - A_param/(2*sqrt2*B_param)*(2*sum_xaij/a_mix - b_coef(i)/b_mix) ...
            * log((Z_eos+(1+sqrt2)*B_param)/(Z_eos+(1-sqrt2)*B_param));
    end
...
if strcmp(eos_type, 'SRK')
    ...
    % alpha = exp((Aa + Ba*Tr) * (1 - Tr^(Ca + Da*omega + Ea*omega^2)));
...
elseif length(valid_roots) == 1
    ...
    Z_V = max(valid_roots);
    if strcmp(eos_type, 'SRK')
        g_L = (Z_L - 1) - log(Z_L - B) - (A/B)*log(1 + B/Z_L);
        g_V = (Z_V - 1) - log(Z_V - B) - (A/B)*log(1 + B/Z_V);
    else
        s2 = sqrt(2);
        g_L = (Z_L - 1) - log(Z_L - B) - A/(2*s2*B)*log((Z_L+(1+s2)*B)/(Z_L+(1-s2)*B));
        g_V = (Z_V - 1) - log(Z_V - B) - A/(2*s2*B)*log((Z_V+(1+s2)*B)/(Z_V+(1-s2)*B));
    end
    if g_L <= g_V
        Z = Z_L;
    else
