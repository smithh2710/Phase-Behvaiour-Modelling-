%%
%comparison script for various thermal diffusion methods for reservoir 1 ,
%using 204 m as ref , PR EOS , no VT 
%%
clear; clc; close all;
...
% comp_ref = [0.42; 0.69; 50.03; 7.848; 6.769; 1.04; 3.199; 1.16; 1.55; 1.88;3.499; 3.749; 2.28; 3.259; 2.589; 2.929; 1.46; 1.65; 1.17; 1.24;0.96; 0.63];
% comp_ref = comp_ref / sum(comp_ref);
% M_gmol = [28.0; 44.0; 16.0; 30.1; 44.1; 58.1; 58.1; 72.2; 72.2; 86.2;96.0; 107.0; 121.0; 140.1; 167.6; 204.7; 243.6; 275.3; 317.0;370.4; 456.8; 640.9];
% Tc = [-147.0; 31.1; -82.5; 32.2; 96.6; 134.9; 152.1; 187.2; 196.4;234.2; 269.9; 290.9; 315.3; 345.9; 385.0; 432.7; 476.8; 511.1;553.3; 604.9; 683.3; 847.8] + 273.15;
% Pc = [33.94; 73.76; 46.00; 48.84; 42.46; 36.48; 38.00; 33.84; 33.74;29.69; 29.60; 27.86; 25.83; 23.76; 21.58; 19.59; 18.24; 17.54;16.84; 16.23; 15.62; 14.72] * 1e5;
% acentric = [0.040; 0.225; 0.008; 0.098; 0.152; 0.176; 0.193; 0.227; 0.251;0.296; 0.338; 0.374; 0.420; 0.483; 0.570; 0.685; 0.795; 0.881;0.984; 1.098; 1.229; 1.159];
% c_JY = [-4.23; -1.91; -5.20; -5.79; -6.35; -7.18; -6.49; -6.20; -5.12;1.42; 8.45; 9.20; 10.32; 11.12; 10.48; 6.57; -1.30; -9.89;-23.43; -43.30; -79.96; -151.44];
% h_ref = 175;
% press_ref = 284e5;
% temp_ref = 93 + 273.15;
...
lg = legend([h_exp_P; h_exp_Psat; h_field_GOC; h_psat_dummy; h_lines], ...
    [{'Measured P', 'Measured P_{sat}', sprintf('Field GOC (%d m)', GOC_field), ...
      'P_{sat} (model, thin)'}, leg_str], 'Location', 'best');
fmt_legend(lg); lg.ItemTokenSize = [22, 10];

print(fig1, 'Res1_Fig1_pressure', '-dpdf', '-r600');
print(fig1, 'Res1_Fig1_pressure', '-dpng', '-r600');
...
h_exp = plot(exp_rho, exp_h, 'ko', 'MarkerSize', mk_sz, 'MarkerFaceColor', 'k', 'LineWidth', 0.8);
h_field_GOC = yline(GOC_field, '-', 'Color', [0.4 0.4 0.4], 'LineWidth', 1.4);
...
lg = legend([h_exp; h_field_GOC; h_lines], ...
    [{'Measured', sprintf('Field GOC (%d m)', GOC_field)}, leg_str], 'Location', 'best');
fmt_legend(lg); lg.ItemTokenSize = [22, 10];

print(fig2, 'Res1_Fig1_density', '-dpdf', '-r600');
print(fig2, 'Res1_Fig1_density', '-dpng', '-r600');
...
print(fig3, 'Res1_Fig1_C1', '-dpdf', '-r600');
print(fig3, 'Res1_Fig1_C1', '-dpng', '-r600');
...
print(fig4, 'Res1_Fig1_C7p', '-dpdf', '-r600');
print(fig4, 'Res1_Fig1_C7p', '-dpng', '-r600');
