...
%% GOC detection
[GOC_h, ~, ~, ~] = detect_sgoc(1, comp_ref, press_ref, temp_ref, h_ref, dTdh, ...
...
h_oil = unique([GOC_h; GOC_h+10; oil_measured]);
h_oil = sort(h_oil);

gas_measured = measured_depths(measured_depths < GOC_h - 2);
h_gas = unique([GOC_h; GOC_h-5; GOC_h-10; gas_measured; 3600]);
...
for i = 1:n_oil
    try
        [ch, Ph, Th, Pb, ~] = main_hasse(h_oil(i), h_ref, comp_ref, press_ref, temp_ref, dTdh, ...
...
%% At saturated GOC: Pbub = Pdew = Pres
oil_Pbub(1) = oil_P(1);
...
for i = 1:n_gas
    try
        [ch, Ph, Th, ~, Pd] = main_hasse(h_gas(i), GOC_h, comp_gas_ref, oil_P(1), oil_T(1), dTdh, ...
            Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vt_method, vt_params);
        gas_comp(i,:) = ch(:)';
        gas_P(i) = Ph; gas_T(i) = Th; gas_Pdew(i) = Pd;
        if h_gas(i) < 3620
            gas_Pdew(i) = NaN;
        elseif isnan(Pd) && all(ch > 0)
            try
...
legend([h1 h2], {'Isothermal, no VT','Measured (Pedersen 2006)'}, ...
