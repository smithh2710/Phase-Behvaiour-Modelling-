%%
% Reservoir 1 — Firoozabadi model only, PR (no VT) vs PR + Abudour VT
% h_ref = 204 m
%%
clear; clc; close all;
...
vt_ids   = [0, 5];
vt_names = {'PR (no VT)', 'PR + Abudour VT'};
n_vt = length(vt_ids);
...
[GOC_h, ~, ~, GOC_T] = detect_sgoc(3, comp_ref, press_ref, temp_ref, h_ref, dTdh, [0 175], Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, 0, vt_params, tau)

%%
for m = 1:n_vt
    vt_method = vt_ids(m);
    fprintf('\n=== Running Firoozabadi + %s (VT=%d) ===\n', vt_names{m}, vt_method);
    ...
    [GOC_h, ~, ~, GOC_T] = detect_sgoc(3, comp_ref, press_ref, temp_ref, h_ref, dTdh, [0 175], Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vt_method, vt_params, tau);
    fprintf('  GOC = %.3f m\n', GOC_h);
    ...
    for i = 1:n_oil
        [ch,Ph,Th,Pb,~] = main_firoozabadi(h_oil(i), h_ref, comp_ref, press_ref, temp_ref, dTdh, Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, tau, vt_method, vt_params);
    ...
    for i = 1:n_gas
        [ch,Ph,Th,~,Pd] = main_firoozabadi(h_gas(i), GOC_h, comp_gas_ref, oil_P(1), GOC_T, dTdh, Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, tau, vt_method, vt_params);
...
%% Experimental (Pedersen 2015, Table 1; densities from thesis Table 3.1)
...
exp_rho  = [285.0; 618.6; 621.2; 627.6; 645.0];
...
print(fig1, 'Res1_Firoozabadi_pressure', '-dpdf', '-r600');
print(fig1, 'Res1_Firoozabadi_pressure', '-dpng', '-r600');
...
print(fig2, 'Res1_Firoozabadi_density', '-dpdf', '-r600');
print(fig2, 'Res1_Firoozabadi_density', '-dpng', '-r600');
...
print(fig3, 'Res1_Firoozabadi_C1', '-dpdf', '-r600');
print(fig3, 'Res1_Firoozabadi_C1', '-dpng', '-r600');
...
print(fig4, 'Res1_Firoozabadi_C7p', '-dpdf', '-r600');
print(fig4, 'Res1_Firoozabadi_C7p', '-dpng', '-r600');

%% Summary
fprintf('\n======== Firoozabadi GOC Summary ========\n');
fprintf('%-22s  %8s\n', 'VT Method', 'GOC (m)');
fprintf('%s\n', repmat('-', 1, 33));
for m = 1:n_vt
