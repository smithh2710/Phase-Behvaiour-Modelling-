...
% =========================================================================
    ...
    try 
        [pressbub_h, ~] = pressbub_multicomp_newton(comp_h, press_h, temp_h, Pc, Tc, acentric, BIP, tol, maxiter, eos_type, components);
    catch 
       pressbub_h = NaN; 
    end
    ...
    try 
        [pressdew_h, ~] = pressdew_multicomp_newton(comp_h, press_h, temp_h, Pc, Tc, acentric, BIP, tol, maxiter, eos_type, components);
    catch 
       pressdew_h = NaN; 
    end
   ...
   [c_ref, fugcoef_internal] = calc_vt(comp_ref, P, T, Pc, Tc, acentric, M_gmol, vt_type, eos_type, vt_opts);
    ...
    for i = 1:n
        ...
        if isscalar(tau)
            ...
            sum_xj_dUj_tauj = sum(comp_ref .* delta_U ./ tau(:));
            Q_i_net = -delta_U(i) / tau(i) + sum_xj_dUj_tauj * v_partial(i) / V_m;
        end
...
function F = residual_firoozabadi(x, params)
    ...
    if vt_type ~= 0 && vt_type ~= 7
        [c_h, fugcoef_internal] = calc_vt(z_h_norm, P_h, temp_h, Pc, Tc, acentric, M_gmol, vt_type, eos_type, vt_opts);
        if ~isempty(fugcoef_internal)
...
function [delta_U, delta_U_mix, v_partial, V_m] = calc_internal_energy_departure(T, P, comp, Pc, Tc, acentric, BIP, M_gmol, vt_type, eos_type, vt_opts)
    ...
    delta_U = zeros(n, 1);
    for i = 1:n
        delta_U(i) = H_res(i) - P * v_partial_eos(i) + R * T;
...
function v_partial = calc_partial_molar_volumes(T, P, comp, Pc, Tc, acentric, BIP, eos_type)
    ...
    for i = 1:n
        if phi_P_plus(i) > 0 && phi_P_minus(i) > 0
            ...
            v_partial(i) = v_ideal + R * T * dln_phi_dP;
        else
            v_partial(i) = v_ideal;
        end
        if isnan(v_partial(i)) || isinf(v_partial(i)) || abs(v_partial(i)) > 1
            v_partial(i) = v_ideal;
        end
    end
...
function H_res = calc_residual_enthalpy(T, P, comp, Pc, Tc, acentric, BIP, eos_type)
    ...
    for i = 1:n
        ...
        if isnan(H_res(i)) || isinf(H_res(i)) || abs(H_res(i)) > 1e8
            H_res(i) = 0;
        end
    end
