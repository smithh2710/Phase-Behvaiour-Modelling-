...
vm = 7;
vp = struct('c_custom', zeros(n,1), 'Vc', Vc, 'components', {components}, 'Zc', Zc);

h_extra_oil = [3655; 3661.6; 3670; 3676.0; 3682.8];
h_extra_gas = [3644.3; 3642; 3638.2; 3630];
...
oil_Pbub = NaN(n_oil,1);

for i = 1:n_oil
    try
        [ch, Ph, Th, Pb, ~] = main_hasse(h_oil(i), h_ref, comp_ref, P_ref, T_ref, dTdh, ...
...
gas_Pdew = NaN(n_gas,1);

for i = 1:n_gas
    try
        [ch, Ph, Th, ~, Pd] = main_hasse(h_gas(i), GOC_h, comp_gas_GOC, P_GOC, GOC_T, dTdh, ...
            Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vm, vp);
        gas_P(i) = Ph; gas_T(i) = Th; gas_comp(i,:) = ch(:)';
        gas_Pdew(i) = Pd;
        if isnan(Pd) && all(ch > 0)
            try
                [Pd_ss, ~] = pressdew_multicomp_ss(ch(:), Ph, Th, Pc, Tc, acentric, BIP, 1e-10, 500, 'SRK', components);
                gas_Pdew(i) = Pd_ss;
            catch
            end
        end
...
legend([h1 h2], {'SRK (no VT)','Measured (Pedersen 2006)'}, ...
