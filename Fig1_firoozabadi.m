...
Vc = (0.29056 - 0.08775 .* acentric) .* R .* Tc ./ Pc;
...
%% GOC detection
[GOC_h, ~, ~, ~] = detect_sgoc(3, comp_ref, press_ref, temp_ref, h_ref, dTdh, ...
...
h_oil = unique([GOC_h; GOC_h+10; GOC_h+15; GOC_h+20; oil_measured]);
h_oil = sort(h_oil);

gas_measured = measured_depths(measured_depths < GOC_h - 2);
h_gas = unique([GOC_h; GOC_h-5; GOC_h-10; GOC_h-15; GOC_h-20; gas_measured; 3600]);
...
for i = 1:n_oil
    try
        [ch, Ph, Th, Pb, ~] = main_firoozabadi(h_oil(i), h_ref, comp_ref, press_ref, temp_ref, dTdh, ...
...
%% At saturated GOC: Pbub = Pdew = Pres (thermodynamic definition)
oil_Pbub(1) = oil_P(1);

%% Flash at GOC to get equilibrium vapor
[~, comp_gas_ref] = pressbub_multicomp_ss(oil_comp(1,:)', oil_P(1), oil_T(1), ...
...
for i = 1:n_gas
    try
        [ch, Ph, Th, ~, Pd] = main_firoozabadi(h_gas(i), GOC_h, comp_gas_ref, oil_P(1), oil_T(1), dTdh, ...
            Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, tau, vt_method, vt_params);
        gas_comp(i,:) = ch(:)';
