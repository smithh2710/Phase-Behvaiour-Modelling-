...
dTdh_vals = [0; 0.01; 0.02; 0.03; 0.04];
ng        = length(dTdh_vals);
labels    = {'dT/dh = 0 K/m (isothermal)', ...
             'dT/dh = 0.01 K/m', ...
             'dT/dh = 0.02 K/m', ...
             'dT/dh = 0.03 K/m', ...
             'dT/dh = 0.04 K/m'};
...
for g = 1:ng
    ...
    for i = 1:nd
        try
            ...
            C1_all(i,g)  = comp_h(idx_C1) * 100;
            C7p_all(i,g) = sum(comp_h(idx_C7p)) * 100;
            try
...
rho_ratio = NaN(nd, ng);
for g = 1:ng
    if ~isnan(rho_all(1,g)) && rho_all(1,g) > 0
        rho_ratio(:,g) = rho_all(:,g) / rho_all(1,g);
    end
end
...
hold(ax1,'on');
for g = 1:ng
    plot(ax1, C1_all(:,g), depths, lstyles{g}, ...
        'Color', colors(g,:), 'LineWidth', lw, 'DisplayName', labels{g});
end
fmt_ax(ax1);
...
hold(ax2,'on');
for g = 1:ng
    plot(ax2, C7p_all(:,g), depths, lstyles{g}, ...
        'Color', colors(g,:), 'LineWidth', lw, 'DisplayName', labels{g});
end
fmt_ax(ax2);
...
for g = 1:ng
    v = ~isnan(rho_ratio(:,g));
    plot(ax3, rho_ratio(v,g), depths(v), lstyles{g}, ...
        'Color', colors(g,:), 'LineWidth', lw, 'DisplayName', labels{g});
end
